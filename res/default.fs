...
void main(){
	...
	vec3 amb_color=vec3(0.05,0.05,0.05)*diff_color;
	...
	vec3 diffvec=pos_world-torch_pos;
	vec3 down_cam=vec3(0,-1,0);
	if(dot(diffvec,down_cam)>0){
		vec3 proj=dot(diffvec,down_cam)*down_cam;
		if(length(proj)>0.8){
		proj=diffvec-proj;
			if(length(proj)<0.25&&length(proj)>0.225){
				color+=vec3(0.15,0.4,0.4);
			}
		}
	}
