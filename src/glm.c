...
Mat4 GLM_viewMat4(Vec3 pos,Vec3 back,Vec3 up){
        ...
        newY=GLM_unitVec3(GLM_crossVec3(GLM_crossVec3(newZ, up), newZ));
...
Mat4 GLM_eulerMat4(float y, float p, float r){
	Mat4 toret;
	toret=GLM_rotateMat4(GLM_createVec3(0, 0, 1), r);
	toret=GLM_mulMat4Mat4(GLM_rotateMat4(GLM_createVec3(1, 0, 0), p), toret);
	toret=GLM_mulMat4Mat4(GLM_rotateMat4(GLM_createVec3(0, 1, 0), y), toret);
	return toret;
}
...
Mat4 GLM_perspMat4(float fov,float aspectRatio,float zmin,float zmax){
        Mat4 toret=GLM_identityMat4();
        toret.e[0][0]=1/tan(GLM_toRadian(fov/2));
        toret.e[1][1]=aspectRatio/tan(GLM_toRadian(fov/2));
        toret.e[2][2]=(zmin+zmax)/(zmax-zmin);
        toret.e[2][3]=2*zmin*zmax/(zmax-zmin);
        toret.e[3][2]=-1;
        toret.e[3][3]=0;
        return toret;
}
